        ...
        while (index < value.length) {
            ...
            if (Lexer.SQUOTE(ch)) {
                ...
                if (!Lexer.SQUOTE(next)) {
                    if (Lexer.pcharNoSQUOTE(value, next) > index && !Lexer.CLOSE(value[index]) && Lexer.RWS(value, index) == index)
                        return;
                    break;
                }
                else {
...
function dateTimeOffsetValue(value, index) {
    ...
    if (hourNext == dayNext + 1 || !Lexer.COLON(value[hourNext]))
        return;
    ...
    if (value[end] == 0x5a) {
        ...
        if (zHourNext == end + 1 || !Lexer.COLON(zHourNext))
            return;
        var zMinuteNext = Lexer.minute(value, zHourNext + 1);
        if (zMinuteNext == zHourNext + 1)
            return;
    }
    else
        return;
...
function timeOfDayValue(value, index) {
    ...
    if (hourNext == index || !Lexer.COLON(value[hourNext]))
        return;
    var minuteNext = Lexer.minute(value, hourNext + 1);
    if (minuteNext == hourNext + 1)
        return;
