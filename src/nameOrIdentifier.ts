...
export namespace NameOrIdentifier {
  ...
  export function singleEnumValue(value: Utils.SourceArray, index: number): Lexer.Token {
    return (
      NameOrIdentifier.enumerationMember(value, index) ||
      ...
      metadataContext.dataServices.schemas.forEach(
        (schema) =>
          ...
          schema.entityTypes.filter((type) => {
            const eq = type.name === entitySet.entityType.replace(schema.namespace + '.', '');
  ...
  export function singletonEntity(value: Utils.SourceArray, index: number): Lexer.Token {
    return NameOrIdentifier.odataIdentifier(value, index, Lexer.TokenType.SingletonEntity);
  }
  ...
  export function typeDefinitionName(value: Utils.SourceArray, index: number): Lexer.Token {
    return NameOrIdentifier.odataIdentifier(value, index, Lexer.TokenType.TypeDefinitionName);
  }
  export function enumerationTypeName(value: Utils.SourceArray, index: number): Lexer.Token {
    return NameOrIdentifier.odataIdentifier(value, index, Lexer.TokenType.EnumerationTypeName);
  }
  export function enumerationMember(value: Utils.SourceArray, index: number): Lexer.Token {
    return NameOrIdentifier.odataIdentifier(value, index, Lexer.TokenType.EnumerationMember);
  }
  export function termName(value: Utils.SourceArray, index: number): Lexer.Token {
    return NameOrIdentifier.odataIdentifier(value, index, Lexer.TokenType.TermName);
  }
  ...
  export function isPrimitiveTypeName(type: string, metadataContext?: any): boolean {
    ...
    const schema = schemas.filter(function (it) {
      return type.indexOf(it.namespace + '.') === 0;
    })[0];
    if (schema) {
      return (
        ((schema.enumTypes &&
          schema.enumTypes.filter(function (it) {
            return it.name === type.split('.').pop();
          })[0]) ||
          (schema.typeDefinitions &&
            schema.typeDefinitions.filter(function (it) {
              return it.name === type.split('.').pop();
            })[0])) &&
        !(
          (schema.entityTypes &&
            schema.entityTypes.filter(function (it) {
              return it.name === type.split('.').pop();
            })[0]) ||
          (schema.complexTypes &&
            schema.complexTypes.filter(function (it) {
              return it.name === type.split('.').pop();
            })[0])
        )
      );
      ...
      for (let i = 0; i < metadataContext.properties.length; i++) {
        ...
        if (prop.name === token.raw) {
          if (
            ...
            !NameOrIdentifier.isPrimitiveTypeName(prop.type, metadataContext)
          )
            return;
          token.metadata = prop;
          ...
          if (
            metadataContext.key &&
            metadataContext.key.propertyRefs.filter((it) => it.name === prop.name).length > 0
          ) {
  ...
  export function primitiveKeyProperty(
    ...
    const token = NameOrIdentifier.primitiveProperty(value, index, metadataContext);
  ...
  export function action(
    ...
    if (typeof metadataContext === 'object') {
      const type = NameOrIdentifier.getOperationType(
        'action',
        metadataContext,
        token,
        isCollection,
  ...
  export function entityFunctionImport(
    ...
    if (typeof metadataContext === 'object') {
      const type = NameOrIdentifier.getOperationImportType(
        'function',
        metadataContext,
        token,
        false,
